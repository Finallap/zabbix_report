<?php
	...
	class  Excel_out_library
	{
	    ...
	    public function all_record_excel_out_other($data = NULL,$header_data,$time_array)
	    {
	    	$name = $header_data['item_name']."报表——".$header_data['start_month'].'至'.$header_data['end_month'].'(导出时间：'.date('Y-m-d H:i:s',time()).')';
	        ...
	        $objPHPExcel->createSheet();
	        $objPHPExcel->getActiveSheet()->setTitle('数据总表');
	        $objPHPExcel->setActiveSheetIndex(0);
			...
			{
				$objPHPExcel->setactivesheetindex(0);
				$objPHPExcel->getActiveSheet()->getColumnDimension($column)->setWidth(17);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue($column.'1', $time_result['date']."平均值");
				$column = chr(ord($column)+1); 

				$objPHPExcel->setactivesheetindex(0);
				$objPHPExcel->getActiveSheet()->getColumnDimension($column)->setWidth(17);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue($column.'1', $time_result['date']."最大值");
				$column = chr(ord($column)+1); 

				$objPHPExcel->setactivesheetindex(0);
				$objPHPExcel->getActiveSheet()->getColumnDimension($column)->setWidth(17);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue($column.'1', $time_result['date']."最小值");
				$column = chr(ord($column)+1); 
			}
			...
			foreach ($data as $key => $host_value)
			{
				...
				foreach ($host_value['detail'] as $key => $value)
				{
					$objPHPExcel->setActiveSheetIndex(0)->setCellValue($column.$row, $value['avg_value']);
					$column = chr(ord($column)+1);
					$objPHPExcel->setActiveSheetIndex(0)->setCellValue($column.$row, $value['max_value']);
					$column = chr(ord($column)+1);
					$objPHPExcel->setActiveSheetIndex(0)->setCellValue($column.$row, $value['min_value']);
					$column = chr(ord($column)+1);
				}
